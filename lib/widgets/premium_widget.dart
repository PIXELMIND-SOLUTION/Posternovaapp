...
//                       builder: (context, provider, child) {
//                         if (provider.isLoading) {
//                           print("hdfkflfjfdfldfjdfjdfld;ffldf;dl${provider.isLoading}");
//                           return const Center(
//                             child: Column(
//                               mainAxisSize: MainAxisSize.min,
//                               children: [
...
//                         if (provider.plans.isNotEmpty) {
//                                                   print("hdfkflfjfdfldfjdfjdfld;ffldf;dl99999999${provider.isLoading}");
...
class _SubscriptionPlansWidgetState extends State<SubscriptionPlansPage> {
  ...
  void initState() {
    ...
    // Defer provider fetch to after first frame to avoid "called during build" error
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!_didFetch && mounted) {
        _didFetch = true;
        final planProvider = context.read<GetAllPlanProvider>();
        if (planProvider.plans.isEmpty && !planProvider.isLoading) {
          planProvider.fetchAllPlans();
        }
      }
  ...
  @override
  Widget build(BuildContext context) {
    // Outer background & decoration (kept from original)
    return Container(
      width: MediaQuery.of(context).size.width,
      height: MediaQuery.of(context).size.height,
      decoration: BoxDecoration(
        ...
        children: [
          ...
          Column(
            children: [
              ...
              // Plans container (expanded to take remaining space)
              Expanded(
                child: Container(
                  ...
                  // Consumer builds the inner content based on provider state
                  child: Consumer<GetAllPlanProvider>(
                    builder: (context, provider, child) {
                      if (provider.isLoading) {
                        return const Center(
                      ...
                      // When plans are available, use a ListView so it scrolls naturally.
              ...
              // (Optional) footer or small spacing if you want
              const SizedBox(height: 8),
            ],
          ),
        ],
...
class _PlanCardState extends State<PlanCard> {
  ...
  Widget build(BuildContext context) {
    ...
    // Wrap in Material so Checkbox (and other Material widgets) have a Material ancestor.
    // Use elevation and rounded corners to preserve the card look.
    return Material(
      color: Colors.white,
      elevation: 2,
      borderRadius: BorderRadius.circular(16),
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          // No need to set color here (Material provides it),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: widget.index == 0 ? const Color(0xFF6366F1) : Colors.grey.shade200,
            width: widget.index == 0 ? 2 : 1,
          ),
        ),
        child: Column(
          ...
          children: [
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                      color: widget.index == 0 ? const Color(0xFF6366F1) : Colors.black87,
                    ),
                  ),
                ),
                const SizedBox(width: 8),
                Text(
                  '${(plan.offerPrice ?? 0).toStringAsFixed(2)} INR',
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: widget.index == 0 ? const Color(0xFF6366F1) : Colors.black87,
            ...
            const SizedBox(height: 8),
            Text(
              '${(plan.duration ?? 'month') == 'month' ? 'Per month' : 'Per year'}',
                    ...
                    child: Row(
                      children: [
                        Icon(
                          Icons.check_circle,
                          size: 14,
                          color: widget.index == 0 ? const Color(0xFF6366F1) : Colors.green,
                        ),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
            ...
            // Terms + Checkbox:
            // Because the whole card is already a Material, Checkbox has a Material ancestor.
            Row(
              children: [
                // You can also wrap only the Checkbox in Material if you prefer:
                // Material(child: Checkbox(...))
                Checkbox(
                  value: _agreedToTerms,
                  onChanged: (val) => setState(() => _agreedToTerms = val ?? false),
                  activeColor: widget.index == 0 ? const Color(0xFF6366F1) : const Color(0xFF8B5CF6),
                ),
                Expanded(
                  child: RichText(
                    text: TextSpan(
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _agreedToTerms
                    ? () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => PlanDetailsAndPaymentScreen(plan: plan),
                          ),
                        );
                      }
                    : null,
                style: ElevatedButton.styleFrom(
                  backgroundColor: widget.index == 0 ? const Color(0xFF6366F1) : const Color(0xFF8B5CF6),
                  foregroundColor: Colors.white,
                  disabledBackgroundColor: Colors.grey.shade300,
                  disabledForegroundColor: Colors.grey.shade500,
                  padding: const EdgeInsets.symmetric(vertical: 12),
                ...
                child: const Text('Purchase Plan', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
              ),
            ),
          ],
        ),
