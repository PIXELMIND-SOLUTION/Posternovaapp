...
class _StoryViewerScreenState extends State<StoryViewerScreen>
  ...
  // Video player - using ExoVideoPlayer
  bool _isVideo = false;
  Key? _videoKey;
  ...
  Future<void> _initializeCurrentMedia() async {
    ...
    // Generate unique key for video player to force rebuild
    _videoKey = ValueKey(
      '${story.id}_${DateTime.now().millisecondsSinceEpoch}',
    );

    if (_isVideo) {
      // For videos, we'll use ExoVideoPlayer with autoPlay
      // The video will start playing automatically when the widget builds
      ...
      body: GestureDetector(
        ...
        child: Stack(
          children: [
            ...
            // Pause indicator
            if (_isPaused && !_isShowingUserOptions && !_isLoading)
              Positioned.fill(
                child: Center(
                  child: Container(
                    width: 60,
