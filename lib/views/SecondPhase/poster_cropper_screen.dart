...
//       body: Column(
//         children: [
//           // ── Cropper fills all remaining space ──────────────────
//           Expanded(
//             child: LayoutBuilder(
//               builder: (context, constraints) {
//                 return ExtendedImage.file(
//                   widget.imageFile,
//                   // Fill the full available area so no dead space
...
//                     return EditorConfig(
//                       maxScale: 8.0,
//                       // No padding — editor uses every pixel
//                       cropRectPadding: EdgeInsets.zero,
//                       hitTestSize: 20.0,
//                       cropAspectRatio: _posterAspectRatio,
//                       // Let the crop rect initialise from the image bounds
//                       initCropRectType: InitCropRectType.imageRect,
//                       // Editor background stays black
//                       editorMaskColorHandler: (context, pointerDown) =>
//                           Colors.black.withOpacity(pointerDown ? 0.4 : 0.6),
//                     );
//                   },
//                   cacheRawData: true,
//                 );
...
class _PosterCropperScreenState extends State<PosterCropperScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // ── Cropper fills ALL available space — no dead zones ──
          // Expanded(
          //   child: LayoutBuilder(
          //     builder: (context, constraints) {
          //       return ExtendedImage.file(
          ...
          //         width: constraints.maxWidth,
          //         height: constraints.maxHeight,
          //         // Fill stretches the image to occupy the full widget
          //         // bounds, eliminating the black letterbox bars that
          //         // BoxFit.contain produced.
          //         fit: BoxFit.fill,
          //         mode: ExtendedImageMode.editor,
          //         extendedImageEditorKey: _editorKey,
          //         initEditorConfigHandler: (state) {
          ...
          //             hitTestSize: 20.0,
          //             cropAspectRatio: _posterAspectRatio,
          //             // layoutRect initialises the crop to the full
          //             // widget area instead of just the image's
          //             // rendered bounds, so nothing is clipped at start.
          ...
          Expanded(
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                final imageAspect = _posterAspectRatio; // posterW / posterH
                final screenAspect = screenW / screenH;

                // Calculate how much vertical dead space BoxFit.contain adds
                double verticalPad = 0;
                double horizontalPad = 0;

                if (imageAspect < screenAspect) {
                  // Image is taller relative to screen → black bars on left/right
                  final renderedW = screenH * imageAspect;
                  horizontalPad = (screenW - renderedW) / 2;
                } else {
                  // Image is wider relative to screen → black bars on top/bottom
                  final renderedH = screenW / imageAspect;
                  verticalPad = (screenH - renderedH) / 2;
                }
                ...
                return ExtendedImage.file(
                  ...
                  width: screenW,
                  height: screenH,
                  fit: BoxFit.contain, // required by extended_image
                  mode: ExtendedImageMode.editor,
                  extendedImageEditorKey: _editorKey,
                  initEditorConfigHandler: (state) {
                    return EditorConfig(
                      maxScale: 8.0,
                      // Negative padding pulls crop rect outward to fill dead space
                      cropRectPadding: EdgeInsets.symmetric(
